import string

password = input("Enter the password:")

#Setup every character type
upperCase = any([1 if c in string.ascii_uppercase else 0 for c in password ])
lowerCase = any([1 if c in string.ascii_lowercase else 0 for c in password ])
special = any([1 if c in string.punctuation else 0 for c in password ])
digits = any([1 if c in string.digits else 0 for c in password ])

characters = [upperCase, lowerCase, special, digits]

length = len(password)

score = 0;

#Read the file containing passwords
with open('D:\Cyber\Python\common.txt', 'r') as f:
    common = f.read().splitlines()

#exit if the password is commonly used
if password in common:
    print("password was found in a common list")
    exit()

#adding points for password's length
if length > 8:
    score += 1
if length >  12:
    score += 1
if length >  15:
    score += 1

print(f'Password length is {str(length)}, adding {str(score)} points ')

#checking how many character types you have used and how many points you will receive for each character
if sum(characters) > 1:
    score += 1
if sum(characters) > 2:
    score += 1
if sum(characters) > 3:
    score += 1
print(f'Password has {str(sum(characters))} different character types, adding {str(sum(characters)- 1 )} points')

#rating your password depending on your final score
if score < 4:
    print (f"password is quite weak! Score :{str(score)} / 7")
elif score == 4:
    print (f"password is ok! Score :{str(score)} / 7")
elif 4 < score < 6:
    print (f"password is pretty good! Score :{str(score)} / 7")
elif score > 6 :
    print (f"password is strong! Score :{str(score)} / 7")